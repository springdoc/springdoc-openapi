...
/**
 * Checks that the scalar starter leaves forwarded-header handling to the application, so
 * that adding it never makes the whole application start trusting {@code X-Forwarded-*}.
 *
 * @author bnasslahsen
 */
@ActiveProfiles("test")
...
class ScalarForwardedHeaderTest {
	...
	@Test
	void doesNotRegisterForwardedHeaderFilterByDefault() {
		assertThat(applicationContext.getBeanNamesForType(ForwardedHeaderFilter.class)).isEmpty();
		assertThat(applicationContext.getBeanNamesForType(FILTER_REGISTRATION)).isEmpty();
	}

	/**
	 * The application asked for forwarded headers: Spring Boot, not the scalar starter,
	...
	class WhenTheApplicationOptsIn {
		...
		@Test
		void registersForwardedHeaderFilter() {
