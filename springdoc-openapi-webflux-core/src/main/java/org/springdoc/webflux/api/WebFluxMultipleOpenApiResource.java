 ...
 * @author bnasslahsen
 */
public abstract class WebFluxMultipleOpenApiResource implements InitializingBean {
	 ...
	 */
	private Map<String, WebFluxOpenApiResource> groupedOpenApiResources;
	 ...
	 */
	public WebFluxMultipleOpenApiResource(List<GroupedOpenApi> groupedOpenApis,
	...
	private WebFluxOpenApiResource buildWebFluxOpenApiResource(GroupedOpenApi item) {
		if (!springDocConfigProperties.isUseManagementPort() && !ACTUATOR_DEFAULT_GROUP.equals(item.getGroup()))
			...
			return new ActuatorOpenApiResource(item.getGroup(),
					defaultOpenAPIBuilder,
					requestBuilder,
					responseBuilder,
					operationParser,
	 ...
	 */
	protected WebFluxOpenApiResource getOpenApiResourceOrThrow(String group) {
		WebFluxOpenApiResource openApiResource = groupedOpenApiResources.get(group);
